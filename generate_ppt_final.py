...
def add_content(slide, items, top=Inches(1.2), font_size=16):
...
p.alignment = PP_ALIGN.CENTER
set_font(p, size=20, color=RGBColor(80, 80, 80))

# 第 2 页：目录
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "汇报目录")
add_content(slide, ["一、切实加强水务安全生产工作的认识", "", "二、水务安全生产现状", "", "三、面临的形势", "", "四、下一步重点工作", "", "五、常见安全隐患"], font_size=22)

# 第 3 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "一、切实加强水务安全生产工作的认识")
content3 = """（一）书记关于安全生产重要论述核心要义

书记始终将安全生产作为极重的民生大事，纳入总体布局和战略布局统筹推进

核心要牢牢把握六个方面：
1. 树牢红线意识，坚守发展决不能以牺牲人的生命为代价这条不可逾越的底线
2. 健全责任体系，严格落实党政同责、一岗双责、齐抓共管、失职追责要求
3. 压实主体责任，做到安全投入、管理、培训、防护、应急五到位
4. 创新监管方式，提升监管精准度和效能
5. 构建长效机制，实现风险常态化管控、隐患动态化治理
6. 强化责任担当，以时时放心不下的责任感抓细抓实各项工作"""
add_content(slide, content3.split('\n'), font_size=15)

# 第 4 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "（二）核心法律法规与政策文件")
content4 = """水务安全生产工作必须在法治框架下推进

通用层面：
• 《安全生产法》明确企业主体责任、政府监管职责、隐患排查治理和应急处置等基本原则

水利领域：
• 《水法》《防洪法》对水利工程设计、施工、运行、维护全生命周期安全管理作出规范
• 强调事前防控、过程控制、事后处置的闭环管理

省市制度：
• 《安全生产末端落实工作机制》
• 《安全生产违法行为行政处罚规定》等制度"""
add_content(slide, content4.split('\n'), font_size=16)
...
为全市经济社会发展提供坚实的水安全保障"""
add_content(slide, content5.split('\n'), font_size=18)
...
add_content(slide, content6.split('\n'), font_size=17)

# 第 7 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "（二）安全生产特点")
content7 = """基于承德地理环境和水务工作实际，主要呈现四大特点：

一是风险点多面广。我市地处四河之源、两库上游，境内有滦河、潮白河等四大水系，1500 条河流、众多水库和水电站

二是季节性风险突出。冬季面临冰面安全和设施冻损风险；夏季汛期来临，山洪、滑坡、河道行洪等风险加剧

三是工程类型复杂多样。既有水库、水电站等运行类工程，又有河道治理、水利设施提质增效等建设类项目

四是与生态安全紧密关联。作为京津唐重要水源地，直接影响区域生态环境和供水安全"""
add_content(slide, content7.split('\n'), font_size=15)

# 第 8 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "已开展的工作")
content8 = """一是安全生产责任体系逐步完善
• 严格落实三管三必须要求
• 制定领导干部安全生产职责清单、科室和单位职责清单、安全生产工作指导意见
• 构建从业务科室到局属单位、从监管单位到生产经营单位的责任闭环

二是专项整治行动深入开展
• 聚焦工程建设、运行管理等重点领域
• 持续开展治本攻坚三年行动、百日攻坚、冬春集中攻坚、常态化隐患排查、双盲抽查等
• 事故发生率呈稳中有降趋势
...
• 推进水利安全生产风险管控六项机制建设
• 完善应急预案体系，组织开展应急演练"""
add_content(slide, content8.split('\n'), font_size=14)

# 第 9 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "（四）存在的主要问题")
content9 = """监管层面存在四大短板：
1. 责任传导上热下冷。市局部署的安全要求在部分县级部门、基层水管单位落实不到位
...
2. 隐患排查整治不彻底。对排查出的问题整改不及时、不彻底，存在临时应付心态
3. 人员安全素养不足。部分从业人员安全意识淡薄，违规操作现象时有发生"""
add_content(slide, content9.split('\n'), font_size=14)

# 第 10 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "三、面临的形势")
content10 = """（一）工作标准越来越严
国家对省级的安全生产考核改为由党中央负责组织实施，考核规格、标准大幅提升

（二）风险考验越来越大
近年来，极端天气事件呈现趋多趋频趋强趋广态势，加之降雨带北抬，安全生产与自然灾害风险交织叠加
...
add_content(slide, content10.split('\n'), font_size=16)

# 第 11 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "四、下一步重点工作")
content11 = """一是持续健全责任体系
...
三是持续提升防控能力
稳步推进水利安全生产风险管控六项机制建设，常态化开展安全警示教育和业务培训

四是持续完善应急保障
修订完善各类突发事件应急预案，组织常态化开展应急演练，通过实战化演练检验预案、锻炼队伍、提升能力"""
add_content(slide, content11.split('\n'), font_size=15)

# 第 12 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "五、常见安全隐患 - 工程施工环节")
content12 = """（一）水利工程施工环节六大安全隐患
...
add_content(slide, content12.split('\n'), font_size=13)

# 第 13 页
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_title_bar(slide, "五、常见安全隐患 - 设施运行环节")
content13 = """（二）水利设施运行环节五大安全隐患
...
add_content(slide, content13.split('\n'), font_size=15)
