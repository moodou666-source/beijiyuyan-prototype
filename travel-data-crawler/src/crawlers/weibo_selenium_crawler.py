...
if __name__ == "__main__":
    ...
    # 关键词模板
    keywords_template = ["{city}旅游", "{city}打卡", "{city}景点", "{city}攻略"]
    
    crawler = WeiboSeleniumCrawler(cookie=cookie)
    
    all_results = []
    
    print(f"开始采集江苏 {len(cities)} 个城市的数据...\n")
    
    for i, city in enumerate(cities, 1):
        print(f"[{i}/{len(cities)}] 采集 {city} 的数据...")
        
        city_results = []
        for template in keywords_template:
            keyword = template.format(city=city)
            try:
                results = crawler.search(keyword, pages=2)
                city_results.extend(results)
                print(f"  - {keyword}: {len(results)} 条")
            except Exception as e:
        ...
        print(f"  => {city} 共 {len(unique_results)} 条唯一数据\n")
    ...
    # 保存数据
    if all_results:
        crawler.save_data(all_results, "jiangsu_all_cities")
        print("数据已保存")
